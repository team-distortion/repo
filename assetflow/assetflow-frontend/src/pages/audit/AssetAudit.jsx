...
function AuditCycleCard({ audit }) {
  ...
  if (isLoading) return <div className="p-6 text-[var(--color-text-secondary)] text-[14px]">Loading audit details...</div>;
  ...
  return (
    <Card className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[var(--color-border)]">
        ...
        {status !== 'Closed' && (
          <Button onClick={handleClose} disabled={isClosing} variant="secondary">
            <CheckCircle2 className="w-4 h-4 text-[var(--color-primary)]" strokeWidth={1.75} />
      ...
      {flaggedCount > 0 && (
        <div className="bg-[var(--color-warning-tint)] border border-[var(--color-warning)]/40 rounded-xl p-4 flex items-center gap-3 text-[var(--color-warning)]">
      ...
      <div className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl overflow-hidden">
        <table className="w-full text-left border-collapse whitespace-nowrap">
          <thead>
            <tr className="h-11 border-b border-[var(--color-border)] bg-[var(--color-surface-3)]">
              <th className="px-4 text-[12px] font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">Asset Name</th>
              <th className="px-4 text-[12px] font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">Asset Tag</th>
              <th className="px-4 text-[12px] font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">Verification Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--color-border)]">
            {items.map((row) => (
              <tr key={row.id} className="h-14 hover:bg-[var(--color-surface-3)] transition-colors">
                <td className="px-4 text-[14px] font-medium text-[var(--color-text)]">
                  {row.assetName}
                </td>
                <td className="px-4 text-[14px] font-mono text-[var(--color-primary)]">
                  {row.assetTag}
                </td>
                <td className="px-4">
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={3} className="h-24 text-center text-[var(--color-text-tertiary)] text-[14px]">
...
export default function AssetAudit() {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading && <div className="text-[var(--color-text-secondary)] text-[14px] p-8">Loading audit cycles...</div>}
      {error && <div className="text-[var(--color-error)] text-[14px] p-8">Failed to load audit records.</div>}
      ...
      <div className="space-y-6">
        ...
        {audits.length === 0 && !isLoading && (
          <Card className="text-center py-12 text-[var(--color-text-tertiary)] text-[14px]">

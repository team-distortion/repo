...
import { useGetDepartmentsQuery, useCreateDepartmentMutation } from '../../store/apiSlice';
      ...
      <div className="glass-panel rounded-3xl overflow-hidden shadow-2xl border border-slate-700/60 relative bg-slate-800/40">
        ...
        {activeTab === 'Departments' && (
          <div className="animate-in slide-in-from-bottom-4 duration-500">
            {isLoading ? <div className="p-8 text-white">Loading...</div> : (
                      ...
                      <td className="py-5 px-8">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-xs font-bold text-white shadow-md">
                            {(dept.head || dept.head_id || 'U').toString().charAt(0).toUpperCase()}
                          </div>
                          <span className="text-slate-300 capitalize text-sm">{dept.head || 'Unknown'}</span>
                        </div>
                      </td>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              {activeTab === 'Departments' ? (
                <>
                  ...
                  <div>
                    <label className="block text-sm text-slate-300 mb-1">Head Name</label>
                    <input type="text" value={newDept.head} onChange={e => setNewDept({...newDept, head: e.target.value})} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white" />
                  </div>
                </>
              ) : (
